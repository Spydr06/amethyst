...
__syscall syscallret_t _sys_brk(struct cpu_context* __unused, void* addr) {
    ...
    if(!thread->user_break_base) {
        ret._errno = ENOMEM;
        return ret;
    }

    uintptr_t old_break = (uintptr_t) thread->user_break; 
    ...
    if(addr >= USERSPACE_END || addr < thread->user_break_base) {
        ret._errno = EINVAL;
        return ret;
    }
    ...
    ret.ret = old_break;
    thread->user_break = (void*) new_break;
