...
int elf_load(struct vnode* node, void* base, void** entry, char** interpreter, Elf64_auxv_list_t* auxv, void** brk) {
    ...
    auxv->pagesz.a_type = AT_PAGESZ;

    auxv->phent.a_un.a_val = header.e_phentsize;
    auxv->entry.a_un.a_val = header.e_entry + (uintptr_t) base;
    auxv->pagesz.a_un.a_val = PAGE_SIZE;
    ...
    *entry = (void*)(header.e_entry + (uintptr_t) base);
    ...
    for(size_t i = 0; i < header.e_phnum; i++) {
        ...
        switch(phdr->p_type) {
            case PT_INTERP:
                interpreter_phdr = phdrs + i * header.e_phentsize;
                break;
            case PT_PHDR:
                ...
                break;
            case PT_LOAD:
                if((err = load(node, phdrs + i * header.e_phentsize, brk)))
                    goto cleanup;
                break;
            default:
